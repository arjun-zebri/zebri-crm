 ...
 * the "+ Add step" tail ghost was placed at `last top-level y +
...
import { COL_GAP, ROW_GAP, autoLayout } from '@/app/(dashboard)/workflows/[id]/auto-layout'
...
describe('autoLayout', () => {
  ...
  it('puts the tail one row under the trigger when there are no actions', () => {
    const { tailY } = autoLayout([])
    // One ROW_GAP, the same pitch as every other gap in the column. It
    // used to be its own number (160), so the tail ghost sat closer to
    // the last step than the steps sat to each other.
    expect(tailY).toBe(ROW_GAP)
  })

  it('spaces every unplaced node by exactly one row', () => {
    const { actions } = autoLayout([
      row({ id: 'a', position: 100 }),
      row({ id: 'b', position: 200 }),
      // A wide gap in run order must not become a wide gap on screen:
      // `position` is ordering, not pixels. The copilot used to write it
      // straight into `canvas_y`, which is what made one gap on a
      // generated workflow three times the size of its neighbours.
      row({ id: 'c', position: 900 }),
    ])
    expect([actions['a']?.y, actions['b']?.y, actions['c']?.y]).toEqual([
      ROW_GAP,
      ROW_GAP * 2,
      ROW_GAP * 3,
    ])
...
describe('positions that were never really chosen', () => {
  it('lays out a step stored at (0, 0) instead of stacking it', () => {
    ...
    // never chose. Trusting those put a whole workflow on one point.
    const a = row({ id: 'a', position: 100, position_x: 0, position_y: 0 });
