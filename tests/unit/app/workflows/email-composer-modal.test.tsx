...
import { EmailComposerModal } from '@/app/(dashboard)/workflows/[id]/email-composer-modal'
...
describe('the review toggle', () => {
  /** Render with the step's "Ask me before this runs" flag attached. */
  function openWithReview(checked = false) {
  ...
  it('is absent when the step cannot hold', () => {
    // A composer opened without the flag (a template editor, say) must
    // not offer a control that writes nowhere.
    render(<EmailComposerModal isOpen onClose={() => {}} config={{}} onSave={vi.fn()} />)
    expect(screen.queryByRole('switch', { name: /Ask me before this runs/ })).toBeNull()
  })

  it('opens showing the saved value', () => {
    openWithReview(true)
    expect(screen.getByRole('switch', { name: /Ask me before this runs/ })).toHaveAttribute(
      'aria-checked',
      'true',
    )
  })

  it('writes the flag on Save, not on the click', () => {
    // It saves to the row rather than to `config`, so nothing else
    // would stop a flipped toggle sticking through a Cancel.
    const onChange = openWithReview()
    fireEvent.click(screen.getByRole('switch', { name: /Ask me before this runs/ }))
