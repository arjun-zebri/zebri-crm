...
describe('readiness', () => {
  it('invoice missing totals raises missing-required issue naming the missing type', () => {
    const blocks: Block[] = [
      { id: '1', type: 'title', title: 'Invoice', subtitle: '', showRef: true, showExpires: false, showAbn: false },
      { id: '2', type: 'lineItems' },
      { id: '3', type: 'action', primary: 'Pay now', secondary: null },
    ]
    const account = { stripeConnected: true, bankDetailsFilled: false, contractTemplateExists: false }
    const result = evaluateSurface('invoice', blocks, account)
    expect(result.ready).toBe(false)
    expect(result.issues).toHaveLength(1)
    expect(result.issues[0]!.kind).toBe('missing-required')
    expect(result.issues[0]!.message).toContain('Totals')
  })
  ...
  it('Layer B account issues do not set ready to false', () => {
    const blocks: Block[] = [
      ...
      { id: '2', type: 'lineItems' },
      { id: '3', type: 'totals', taxRate: 0.1, showSubtotal: true },
      { id: '4', type: 'paymentDetails', heading: '', accountName: '', bsb: '', accountNumber: '' },
    ]
    ...
    expect(result.issues).toHaveLength(1)
    expect(result.issues[0]!.kind).toBe('account')
    expect(result.issues[0]!.message).toContain('Bank details')
  })
})
