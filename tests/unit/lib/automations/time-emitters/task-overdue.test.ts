/**
 * Unit coverage for the `task_overdue` trigger's match narrowing (A5).
 *
 * The end-to-end behaviour (DB read, dedupe, emit RPC) lives in the
 * matching integration spec at
 * `tests/integration/automations/task-overdue-emitter.test.ts`. These
 * tests pin down the pure logic: which emitted `days_overdue` value a
 * given automation config should fire for, including the "min of 0
 * still means at least 1 day past due" rule. Mirrors the A4
 * `invoice_overdue` spec.
 */

import { describe, expect, it } from 'vitest'

import {
  getTriggerSpec,
  taskOverdueThresholdDays,
} from '@/lib/automations/triggers'
import type { AutomationEventRow } from '@/types/automations'

function makeEvent(daysOverdue: number | string | undefined): AutomationEventRow {
  return {
    id: 'evt',
    user_id: 'u',
    source_table: 'tasks',
    source_id: 't',
    event_type: 'task_overdue',
    payload:
      daysOverdue === undefined
        ? ({} as never)
        : ({ task_id: 't', days_overdue: daysOverdue } as never),
    couple_id: null,
    created_at: new Date().toISOString(),
    processed_at: null,
    error_message: null,
  }
}

describe('taskOverdueThresholdDays()', () => {
  it('defaults to 1 day past due when min is unset', () => {
    expect(taskOverdueThresholdDays({})).toBe(1)
  })

  it('clamps a configured min of 0 up to 1', () => {
    // "Overdue" means strictly past the due date — a task due today is
    // not yet overdue (due_date < today).
    expect(taskOverdueThresholdDays({ daysOverdueMin: 0 })).toBe(1)
  })

  it('passes through a configured min above 1', () => {
    expect(taskOverdueThresholdDays({ daysOverdueMin: 5 })).toBe(5)
  })
})

describe('task_overdue trigger match()', () => {
  const spec = getTriggerSpec('task_overdue')

  it('registry has a spec', () => {
    expect(spec).not.toBeNull()
  })

  it('matches the default threshold (1 day overdue) with an empty config', () => {
    expect(spec!.match(makeEvent(1), {})).toBe(true)
  })

  it('matches when payload.days_overdue equals the configured min', () => {
    expect(spec!.match(makeEvent(3), { daysOverdueMin: 3 })).toBe(true)
  })

  it('rejects when the overdue depth differs from the threshold', () => {
    expect(spec!.match(makeEvent(3), { daysOverdueMin: 7 })).toBe(false)
    expect(spec!.match(makeEvent(7), {})).toBe(false)
  })

  it('ignores a legacy daysOverdueMax in match()', () => {
    // The field left the schema in the trigger sweep (an exact-depth
    // match makes a max either redundant or unsatisfiable). The
    // emitter keeps its own guard for the never-fire window.
    expect(
      spec!.match(makeEvent(5), { daysOverdueMin: 5, daysOverdueMax: 3 }),
    ).toBe(true)
  })

  it('rejects when payload lacks days_overdue', () => {
    expect(spec!.match(makeEvent(undefined), {})).toBe(false)
  })

  it('rejects when payload.days_overdue is non-numeric', () => {
    expect(spec!.match(makeEvent('three'), {})).toBe(false)
  })
})
