...
- A live `video` booking with no `video_join_url` gets one warning line under the countdown ("No video link was created for this booking, so the couple has none. Send them one before the call."). The calendar push can succeed without minting a link (Outlook on an account without Teams, or no calendar at all) and the couple's confirmation already says "link to follow", so the MC is the only one left who can fix it. The same case fires the `booking_video_link_missing` Slack alert with the provider's own answer.
