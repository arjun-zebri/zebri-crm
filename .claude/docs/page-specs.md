...
- **Event date is a real event.** The couple-level `event_date` column is legacy/dead (the schedule lives in `events`), so an imported row with a date creates an `events` row (date + venue) for the couple, which then shows on the calendar and the list. The Add Couple modal has a Wedding date field that does the same via `upsertCoupleEventDateAction` (creates the couple's first event, or updates its soonest).
