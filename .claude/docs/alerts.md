...
| `booking_video_link_missing` | warn | A `video` booking's calendar event was created but the provider returned no Meet/Teams link, so the couple's confirmation says "Video call (link to follow)" and nothing follows unless the MC adds one by hand. Carries the provider's own explanation: Google's `createRequest` status, or the Outlook calendar's `allowedOnlineMeetingProviders` (Graph silently ignores `isOnlineMeeting` for personal accounts, users without a Teams licence, or tenants with the Outlook add-in policy off) | `app/api/booking/submit/route.ts` |
