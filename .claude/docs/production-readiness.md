...
> Status: **Phase 0 → 4** ✅ all on main. **Phase 5 (Contacts)** ✅ on staging. **Phase 6 (Tasks)** ✅ on staging. **Phase 7 (Dashboard)** ✅ on staging. **Phase 8 (Client Portal)** ✅ on staging + main. **Phase 9 (Quotes)** ✅ on staging. **Phase 10 (Timeline)** ✅ on staging. **Phase 11 (Branding)** ✅ on staging. **Phase 12 (Settings)** ✅ on staging. **Phase 13 (Admin + Ops)** ✅ in flight on `phase-13-admin-ops`. **Branding overhaul (all 8 phases)** ✅ shipped on feature/proposals-phase-a (2026-07-17), awaiting CI migration deploy. **Website form (block-based lead surface)** 🚧 in flight on `worktree-lead-form-blocks` (2026-08-13): extends ZEB-2 into a `lead` branding surface with configurable `formField` blocks, block-rendered public page, `form_submissions` storage, and couple creation. **Scheduler Phase A** 🚧 on feature/scheduler-phase-a (2026-08-19): calendar connections foundation (migration, purpose-aware OAuth, `lib/calendar` modules for connections + free-busy, Settings UI). **Scheduler Phase B** 🚧 on feature/scheduler-phase-b (2026-08-19): meeting types CRUD, weekly availability editor with timezone support, date overrides with block-or-custom-window, TimeSelect primitive, /calendar tabs. Stacked on Phase A. Pending PR to staging. **Scheduler Phase C** 🚧 on feature/scheduler-phase-c (2026-08-20): public booking page (/book/[token]), slots + submit APIs, bookings table with exclusion guard, calendar event push with Meet/Teams links, booking emails, e2e spec. Stacked on A+B. Pending PR to staging. **Scheduler Phase D** 🚧 on feature/scheduler-phase-d (2026-08-20): manage page (/book/manage/[manage_token]) for booker reschedule and cancel, lifecycle RPCs (cancel_booking, reschedule_booking, get_booking_by_manage_token), lifecycle emails with manage link, reminder cron (/api/cron/booking-reminders at 22:30 UTC), consultation_completed time emitter, consultation_booked + booking_cancelled + consultation_completed automation triggers. Stacked on C. Pending PR to staging. **Scheduler Phase E** ✅ on feature/scheduler-phase-e (2026-08-20): MC calendar day/week hour grids with availability shading, busy blocks, booking chips, all-day band, legend, layer toggles; sidebar agenda replacing event list; Bookings tab with detail panel (cancel/reschedule via RLS-proven ownership); authenticated /api/calendar/busy with fail-soft posture; public /book redesigned to three-column layout (meta panel, month calendar with day-state circles, slot column) with times in MC timezone and city line. Known open issue: DST week-view booking render failure (suspected column-matching defect). Stacked on C+D. Docs updated. **Proposals engine Phase A** ✅ landed 2026-09-13 on `feature/proposals-engine`: migration (`proposals`, `proposal_options`, `proposal_option_items`, `generate_proposal_number`, `get_public_proposal`), sidebar + list + detail pages, builder modal, couple-profile tab, send-email route, minimal public page at `/proposal/[token]`. Full model: `.claude/docs/proposals.md`. Phases B-E (page mode, acceptance, engagement, workflow triggers) not started. **Proposal Layout v2 Phase 1** ✅ built 2026-09-16 on `feature/proposal-layout-v2` (staging-only, behind `NEXT_PUBLIC_PROPOSAL_LAYOUT_V2`): the `features/proposals/` module (layout model, Zod schema, v1-to-v2 migration, presets, public renderer), `proposal_templates` + `proposal_settings` tables, `proposals.layout`/`template_id` columns, the `get_public_proposal_layout` RPC, and a Templates tab in the (flag-gated) Proposals nav. Spec `docs/superpowers/specs/2026-09-16-proposal-layout-v2-design.md`, plan `docs/superpowers/plans/2026-09-16-proposal-layout-v2-phase1-foundation.md`. **Proposal Layout v2 Phase 2** ✅ built 2026-09-17 on `feature/proposal-layout-v2` (staging-only, same flag), uncommitted in the working tree pending review: the template editor at `/proposals/templates/[id]` (section canvas, add palette, section/node/text control bars, resize engine, mobile canvas preview, autosave) plus `components/editor/` (toolbar and canvas primitives lifted out of Branding so both editors share one set). Plan `docs/superpowers/plans/2026-09-16-proposal-layout-v2-phase2-template-editor.md`. Full model: `.claude/docs/proposals.md` (Layout v2 Phase 2). Known gap: the `proposal-media` bucket's `allowed_mime_types` was never widened for image/audio uploads (see `.claude/docs/security.md`). Phases 3-5 (native data-section shapes, builder design mode + settings + public v2 rollout, analytics) not started. **R1 (pg_cron scheduler + step timing)** ✅ merged to staging 2026-09-21 (PR #98): automations tick moved off Vercel's daily/hourly cron onto `pg_cron` running every 15 minutes, with `system_heartbeats` staleness tracking, a 45-second dispatch deadline across all three passes, an hourly digest, and an Admin Scheduler card (`syncSchedulerAction`) to push Vault secrets. Step timing rewritten on one Zod schema (`lib/workflows/timing-schema.ts`): `minutes` (multiples of 15) / `hours` / `days` amounts plus an optional `sendTime` on the same 15-minute grid for `wedding_relative` and day-based `apply_relative` steps. Roadmap spec `docs/superpowers/specs/2026-09-20-proposals-completion-roadmap-design.md` (R0 to R5), plan `docs/superpowers/plans/2026-09-20-r0-r1-cron-platform-and-timing.md`. **Tick reliability fix** ✅ built 2026-09-22 on `fix/tick-every-minute` after production showed "wait 15 minutes, then send" landing 30 to 45 minutes late: tick every minute (`20261001200000_tick_every_minute.sql`), executor first on its own 30-second slice and chaining zero-delay followers in the same pass, time emitters on the quarter hour only, bus events older than 24 hours stamped `skipped: stale` instead of replayed, `TICK_STALE_MS` down to 5 minutes, a pg_cron `tick_watchdog()` posting to Slack through pg_net independently of the app (`slack_webhook_url` in Vault via Sync scheduler), and Vercel functions pinned to `syd1`.
