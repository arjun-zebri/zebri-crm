...
alter table quotes
  ...
  add column tax_rate numeric(5,2) not null default 10;

-- Add discount fields to invoices (tax_rate already exists)
alter table invoices
  add column discount_type text check (discount_type in ('percentage', 'fixed')),
  add column discount_value numeric(10,2);

-- Update get_public_quote to return discount + tax fields
...
-- Update get_public_invoice to include discount fields
