-- Booking links require a connected calendar.
--
-- A booking accepted while the MC has no working calendar connection never
-- reaches their real calendar and, for video meeting types, promises a join
-- link that nothing ever sends ("Video call (link to follow)"). Both public
-- RPCs now treat every meeting type as unavailable until the owner has a
-- calendar_connections row in status 'connected'. get_public_booking_page
-- returns null (same as a paused type) and submit_booking returns
-- {error:'calendar_required'} so a page loaded before a disconnect cannot
-- complete.
--
-- Both bodies are otherwise verbatim from 20260820001000_booking_rpcs.sql.
...
begin
  ...
  where mt.share_token = token
    and mt.active = true
    -- Booking links only resolve for MCs with a working calendar. Without
    -- one the booking never reaches their real calendar and a video meeting
    -- goes out with no join link, so the link reads as unavailable instead.
    and exists (
      select 1 from calendar_connections cc
      where cc.user_id = mt.user_id and cc.status = 'connected'
    );
...
begin
  ...
  -- Same calendar guard as get_public_booking_page, re-checked here because
  -- the page may have loaded before the MC disconnected. A distinct error so
  -- the route does not count it as an invalid-token probe.
  if not exists (
    select 1 from calendar_connections cc
    where cc.user_id = mt.user_id and cc.status = 'connected'
  ) then
    return '{"error":"calendar_required"}'::jsonb;
  end if;
