-- Add payment feature columns to invoices
alter table invoices
  add column tax_rate               numeric(5,2)  not null default 0,
  add column payment_terms          text          null,
  add column deposit_percent        numeric(5,2)  null,
  add column deposit_due_date       date          null,
  add column deposit_paid_at        timestamptz   null,
  add column final_due_date         date          null,
  add column final_paid_at          timestamptz   null,
  add column stripe_payment_enabled boolean       not null default false,
  add column stripe_payment_intent_id text        null;

-- Update get_public_invoice to include new fields
