...
export function InvoiceLoading({ radius }: InvoiceLoadingProps) {
  return (
    ...
    >
      <div className="h-5 w-24 bg-surface-muted rounded-control animate-pulse" />
      <div className="h-7 w-64 bg-surface-muted rounded-control animate-pulse" />
      <div className="space-y-2 pt-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-10 bg-surface-muted rounded-control animate-pulse" />
        ))}
      </div>
    </div>
  );
