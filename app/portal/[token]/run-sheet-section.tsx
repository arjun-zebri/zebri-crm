  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-stretch gap-2 group">
      ...
      >
        ...
        <div className="pl-4 pr-3 py-2.5 flex items-start gap-3">
          <span className={`text-xs tabular-nums font-medium shrink-0 pt-0.5 w-16 ${
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={items.map((i) => i.id)} strategy={verticalListSortingStrategy}>
            <div className="space-y-2">
              {mcItems.length > 0 && (
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Added by your MC</p>
              )}
              {mcItems.map((item) => (
                <SortableRow key={item.id} item={item} onEdit={handleOpenEdit} />
              ))}
              {pendingItems.length > 0 && (
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mt-3">Your suggestions</p>
              )}
