...
export function HealthBanner({ erroredCount, firstErrorAutomationId, firstErrorAutomationName }: Props) {
  ...
  if (!firstErrorAutomationId) {
    return (
      <div className="bg-warning/10 text-warning text-body px-3 py-2 rounded-control">{body}</div>
    )
  }
  return (
    <Link
      href={`/automations/${firstErrorAutomationId}`}
      className="block bg-warning/10 text-warning text-body px-3 py-2 rounded-control hover:bg-warning/15 transition cursor-pointer"
    >
      {body}
    </Link>
  )
