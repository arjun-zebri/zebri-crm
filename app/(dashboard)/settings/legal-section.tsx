...
export function LegalSection({ title, lastUpdated, canonicalUrl, children }: LegalSectionProps) {
  return (
    <div>
      ...
      <p className="mt-1 text-xs text-text-subtle">Last updated: {lastUpdated}</p>
      <a
        href={canonicalUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="mt-2 inline-flex items-center gap-1.5 text-sm text-text-muted hover:text-gray-700 transition cursor-pointer"
      >
        View the latest at {canonicalUrl.replace(/^https?:\/\//, '')}
        <ExternalLink size={14} strokeWidth={1.5} />
      </a>
      <div className="mt-6 space-y-5 text-sm leading-relaxed text-gray-700 [&_h3]:text-sm [&_h3]:font-semibold [&_h3]:text-text [&_h3]:mt-5 [&_h3]:mb-1.5 [&_p]:text-gray-600 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-1 [&_a]:text-text [&_a]:underline">
        {children}
      </div>
    </div>
  );
