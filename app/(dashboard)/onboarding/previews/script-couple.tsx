  ...
  const modal = show(3) && !show(8) && (
    <Backdrop>
      <PreviewModal
        ...
        footer={
          <>
            <span className="text-body px-3 py-1 rounded-control border border-transparent bg-surface-muted text-text">
              Cancel
            </span>
            <span
              data-cursor="save"
              className="rounded-control px-3 py-1 text-body border border-brand-fg bg-brand-fg text-text-inverse"
            >
              Save
            </span>
          </>
      ...
      >
        ...
        <div>
          ...
          <div className="border-b border-border h-6 flex items-center justify-between">
            <span className="text-body text-text">New</span>
      ...
      <div className="relative h-full flex flex-col">
        {show(1) && (
          <div className="flex items-center justify-between mb-3 pb-2 border-b border-border animate-fade-in">
            <div>
              ...
              <span className="text-body text-text-muted ml-2">{show(8) ? '1' : '0'} total</span>
            </div>
            <span
        ...
        {show(2) && !show(4) && (
          <div className="absolute right-4 top-11 z-10 w-40 rounded-control border border-border bg-card shadow-lg py-1 animate-fade-in">
            <p data-cursor="add-manually" className="px-3 py-1.5 text-body text-text hover:bg-surface-muted">
              Add manually
            </p>
            <p className="px-3 py-1.5 text-body text-text-muted hover:bg-surface-muted">
              Import from CSV
            </p>
          </div>
        )}
          ...
          <div className="flex-1 flex gap-2 overflow-hidden">
            <BoardColumn tone="amber" label="New">
              <div className="rounded-control border border-border bg-card p-3 space-y-2 animate-fade-in">
                <p className="text-body font-semibold text-text">Ellie &amp; Tom</p>
...
function Field({
  ...
  return (
    <div data-cursor={target}>
      ...
      <div className="border-b border-border h-6 flex items-center text-body text-text">
        {typing ? (
