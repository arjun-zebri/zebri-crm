...
function CategorySection({
  ...
  return (
    <div>
      ...
      >
        <h3 className="text-xs font-semibold uppercase tracking-wider text-text group-hover:text-gray-600 transition">{label}</h3>
      ...
      <div className="mt-4 space-y-2.5">
        {isLoading ? (
          ...
          <p className="text-sm text-text-subtle py-2">No people added yet.</p>
        ) : (
          items.map((person) => (
            <div
              key={person.id}
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                {person.phonetic && (
                  <p className="text-sm text-text-muted mt-0.5 font-mono">{person.phonetic}</p>
                )}
              </div>
              {person.audio_url && (
