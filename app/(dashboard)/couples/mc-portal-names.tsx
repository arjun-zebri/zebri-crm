...
const CATEGORIES = [
  { label: 'Partners', category: 'partner', roles: PARTNER_ROLES },
...
export function McPortalNames({ people, onEditPerson, onAddPerson }: McPortalNamesProps) {
  return (
    <div className="space-y-6">
      {CATEGORIES.map(({ label, category, roles }) => {
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {person.role && (
                        <span className="text-xs text-gray-500 bg-gray-100 rounded-full px-2.5 py-0.5">{person.role}</span>
                      )}
                    </div>
                    {person.phonetic && (
