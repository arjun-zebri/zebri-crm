...
function pill(status: string, opts?: { wrap?: boolean }): ReactNode {
  const className = `inline-flex items-center px-2 py-0.5 rounded-pill text-body font-medium capitalize${opts?.wrap === false ? ' whitespace-nowrap' : ''} ${CONTRACT_STATUS_STYLES[status] || CONTRACT_STATUS_STYLES.draft}`;
