...
export default function PaymentsPage() {
  ...
  const { data: invoices, isLoading: invoicesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('invoices')
        .select('id, invoice_number, title, status, subtotal, due_date, created_at, couple:couple_id(id, name)')
        .eq('user_id', user.user.id)
        .order('created_at', { ascending: false })
  ...
  const filteredInvoices = (invoices || [])
    .map((inv) => {
      ...
      return { ...inv, effectiveStatus: isOverdue ? 'overdue' : inv.status, isOverdue }
    })
    .filter((inv) => {
          ...
          <div className="flex items-center gap-1">
            <div className="relative mr-1">
              ...
              <input
                ...
                onChange={(e) =>
                  activeTab === 'quotes' ? setQuoteSearch(e.target.value) : setInvoiceSearch(e.target.value)
                }
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="px-6 pb-6 pt-4">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            {activeTab === 'quotes' ? (
              <>
                <div className="grid grid-cols-[auto_1fr_1fr_120px_100px_100px] gap-0 border-b border-gray-100 sticky top-0 bg-white">
                ...
                {isLoading ? (
                  ...
                  filteredQuotes.map((quote) => (
                    ...
                    >
                      ...
                      <div className="px-4 py-3.5 text-sm text-gray-500 group-hover:text-gray-900 truncate min-w-0">{quote.title}</div>
                      <div className="px-4 py-3.5 text-sm text-gray-500 group-hover:text-gray-900 truncate min-w-0">{quote.couple.name}</div>
                ...
                {isLoading ? (
                  ...
                  filteredInvoices.map((invoice) => (
                    ...
                    >
                      ...
                      <div className="px-4 py-3.5 text-sm text-gray-500 group-hover:text-gray-900 truncate min-w-0">{invoice.title}</div>
                      <div className="px-4 py-3.5 text-sm text-gray-500 group-hover:text-gray-900 truncate min-w-0">{invoice.couple.name}</div>
                      <div className="px-4 py-3.5">
                        <span
                          className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${
