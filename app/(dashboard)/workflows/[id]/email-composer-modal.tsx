 ...
 *
 * @module app/(dashboard)/workflows/[id]/email-composer-modal
 */
'use client'
...
interface Props {
  ...
  showOptions?: boolean
  /**
   * The step's "Ask me before this runs" flag.
   *
   * It lives on the row rather than in `config`, and the card this
   * modal replaced is collapsed for good, so the composer is the only
   * place left to set it. Omitted for a step that cannot hold.
   */
  review?: { checked: boolean; onChange: (checked: boolean) => void }
}
...
export function EmailComposerModal({
  ...
  // Held as a draft like every other field, even though it saves to the
  // row rather than to `config`: a toggle that stuck after Cancel would
  // be the one control on the form that ignores the footer.
  ...
  useEffect(() => {
    ...
    setNeedsReview(review?.checked ?? false)
  ...
  return (
    ...
    >
      <div className="space-y-5">
        {/* First, not last: whether this send goes out by itself is the
            decision the MC most needs to see, and at the foot of a long
            composer it was below the fold and easy to miss. */}
        {review ? (
          <Toggle
            checked={needsReview}
