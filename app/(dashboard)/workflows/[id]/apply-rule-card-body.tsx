 ...
 * per-trigger sweep.
 *
 * @module app/(dashboard)/workflows/[id]/apply-rule-card-body
 */
'use client'
...
export function useTriggerFilters(
  ...
  return useMemo(() => {
    const build = CHIP_TRIGGERS[applyRuleType]
...
export function TriggerCardBody({
  ...
  if (filters.length === 0) {
    return (
      <StepConfigForm
        selection={{ kind: 'trigger', applyRuleType, applyRuleConfig: config }}
        templateId={templateId}
        onSaved={(payload) => {
          if (payload.kind === 'trigger') onConfigChange(payload.applyRuleConfig)
        }}
      />
    )
  }
