 ...
 * logic lives here.
 *
 * @module app/(dashboard)/workflows/[id]/inspector-extended
 */
'use client'
...
import { QuestionnaireChips } from './step-chips'
...
/** Labelled multi-line input. Exported for the manual-step inspector. */
...
/**
 * Labelled select that tolerates an empty value.
 *
 * The shared Select crashes on an option with `value=""`, so an
 * explicit "none" is swapped for a sentinel on the way in and back on
 * the way out. Exported for the manual-step inspector.
 */
...
export function ExtendedTriggerFields({
  applyRuleType, config, setConfig,
}: {
  applyRuleType: TriggerType; config: Cfg; setConfig: SetCfg
}) {
  switch (applyRuleType) {
    ...
    case 'rehearsal_completed':
      return <ConsultationExtra applyRuleType={applyRuleType} config={config} setConfig={setConfig} />
    case 'noim_lodged':
    case 'noim_overdue':
    case 'donlim_due':
    case 'donlim_signed':
    case 'marriage_certificate_issued':
      return <ComplianceExtra applyRuleType={applyRuleType} config={config} setConfig={setConfig} />
    ...
    <>
      ...
      <SelectField
        ...
        options={[
          ...
          { value: 'mc', label: 'MC' },
          { value: 'system', label: 'System / workflow' },
...
function ConsultationExtra({
  ...
  return (
    <>
      ...
      {applyRuleType === 'consultation_booked' && (
      ...
      {applyRuleType === 'booking_cancelled' && (
      ...
      {applyRuleType === 'consultation_completed' && (
      ...
      {applyRuleType === 'rehearsal_scheduled' && (
...
function AutomationFailedExtra({ config, setConfig }: { config: Cfg; setConfig: SetCfg }) {
  return (
    <>
      <TextField label="Specific workflow ID (optional)" value={(config['templateId'] as string) ?? ''} onChange={(v) => setConfig({ ...config, templateId: v || undefined })} placeholder="UUID" />
...
export function ExtendedActionForm({
  ...
  switch (actionType) {
    ...
    case 'goto_step':
      return <TextField label="Step ID" value={(config['stepId'] as string) ?? ''} onChange={(v) => updateConfig({ stepId: v })} placeholder="Step ID from this workflow" />
    case 'send_to_slack':
