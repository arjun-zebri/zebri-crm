 ...
 *
 * @module app/(dashboard)/workflows/[id]/step-summary
 */
...
export function stepTitle(action: AutomationActionRow): string {
  ...
  // members of `ActionType`, but the builder row carries them.
  switch (action.type as string) {
    // Manual step types are not actions, so `actionUi` has nothing for
    // them and the card would read "Step".
    case 'todo':
      return 'To-do'
    case 'appointment':
      return 'Appointment'
...
export function stepSummary(action: AutomationActionRow, labels?: StepSummaryLabels): string {
  ...
  switch (action.type as string) {
    case 'todo':
    case 'appointment':
      // The title IS the step for a manual one, so a titled card needs
      // no second line; an untitled one needs a nudge.
      return action.label ? '' : 'Give it a name'
    case 'wait':
