 ...
 * to fill the gap.
 *
 * @module app/(dashboard)/workflows/[id]/inspector-panel
 */
'use client'
...
import { TriggerFilterList } from './filter-list'
import {
  ApprovalExtraFields,
...
import { TaskComposerModal } from './todo-composer-modal'
...
export type SavedPayload =
  | { kind: 'trigger'; applyRuleConfig: Record<string, unknown> }
  | {
      kind: 'action'
      stepId: string
      config: Record<string, unknown>
      /** The step's title. Only the manual steps can change it. */
      label?: string
    }
 ...
 * card never expands, so the toggle has to travel into the modal or it
...
export const MODAL_ACTIONS: ReadonlySet<string> = new Set([
  ...
  // The manual steps: a name, a note and a due-by rule read as a form,
  // and their card is the one an MC opens most.
  'todo',
  'appointment',
...
function ConfigureTab({
  ...
  if (selection.kind === 'trigger') {
    return (
      <TriggerConfigForm
        templateId={templateId}
        applyRuleType={selection.applyRuleType}
        applyRuleConfig={selection.applyRuleConfig}
        onSaved={onSaved}
      />
    )
  }
...
function TriggerConfigForm({
  ...
  templateId: string
  applyRuleType: TriggerType
  applyRuleConfig: Record<string, unknown>
  onSaved: (payload: SavedPayload) => void
}) {
  const [config, setConfig] = useState<Record<string, unknown>>(applyRuleConfig ?? {})
  ...
  useDebouncedAutosave(config, () => {
    onSaved({ kind: 'trigger', applyRuleConfig: config })
    void setApplyRuleAction({ templateId, applyRuleType, applyRuleConfig: config })
  })
  ...
  return (
    <div className="space-y-3">
      ...
      {applyRuleType === 'custom_field_changed' && (
        <>
          <TextInput
      ...
      {applyRuleType === 'specific_date_reached' && (
        <>
          <DateInput
            label="Date"
      ...
      {applyRuleType === 'portal_section_started_not_finished' && (
        <>
          <SelectInput
      ...
      {NO_CONFIG_HINT_TRIGGERS.has(applyRuleType) && (
      ...
      <ExtendedTriggerFields applyRuleType={applyRuleType} config={config} setConfig={setConfig} />
    </div>
  )
}
...
function ActionConfigForm({
  ...
  // through the same upsert, so it is state here rather than in the
  // composer.
  ...
  // A manual step configures itself entirely in its composer, so the
  // card renders nothing inline: the timing control and the toggles
  // travel into the modal with the fields rather than sitting behind a
  // chevron the card no longer offers.
  if (modal && isManualStep(action.type)) {
    return (
      <ManualStepModal
        isOpen={modal.open}
        onClose={modal.onClose}
        stepType={action.type as unknown as 'todo' | 'appointment'}
        value={{ label, config, timing }}
    ...
    <div className="space-y-3">
      ...
      {automated ? (
        <Toggle
          ...
          label="Ask me before this runs"
          description="It stays in your upcoming tab until you execute manually."
        />
      ) : null}
      ...
      {/* The review toggle is handed over only in modal mode: the
          inline card renders its own copy above, and two of them on one
          form is one too many. */}
      {action.type !== 'wait' && action.type !== 'branch' && action.type !== 'approval' && action.type !== 'sub_flow' && action.type !== 'stop' && !isManualStep(action.type) && (
        <ActionFields
 ...
 */
function useDebouncedAutosave(watched: unknown, fire: () => void) {
  // Compared by content, not by reference. Callers watch a composite of
  // config, timing and the review flag, which is a fresh object every
  // render; a reference comparison would fire a save on every render and
  // the save's own re-render would fire the next one.
  const key = JSON.stringify(watched ?? null)
...
/**
 * `todo` and `appointment` are stored step types, not actions, so they
 * never reach {@link ActionFields}. They configure themselves entirely
 * in {@link ManualStepModal}, which is why this form has no inline
 * branch for them.
 */
function isManualStep(type: string): boolean {
  return type === 'todo' || type === 'appointment'
}

/* ─── Action step dispatch ─────────────────────────────────────── */

/**
 * The fields for one action, by slug.
 *
 * Exported because the same fields edit an *instance* step from the
 * queue's detail modal: one action, one form, wherever it is being
 * configured. Pass no `modal` there - the caller is already one.
 */
export function ActionFields({
  ...
  switch (actionType) {
      ...
      return (
        <>
          <Hint>
            ...
            workflows should use “Send email” with the {'{{portal.link}}'} variable, which
      ...
      return (
        <>
          <Hint>
            ...
            below. New workflows should use “Send email”.
          </Hint>
    ...
    case 'pause_couple_automations':
      ...
      return (
        <Hint>
          Legacy step: stops every other workflow running on this couple. No longer offered
          for new workflows.
        </Hint>
      )
    ...
    case 'send_final_run_sheet':
      ...
      return (
        <Hint>
          ...
          message. New workflows should use “Send run sheet”, which also reaches the
          couple or you.
        </Hint>
      )
      ...
      return (
        <>
          <Hint>
            ...
            workflows should use “Send run sheet”.
          </Hint>
  ...
  return (
    <>
      <Hint>
        ...
        whose ID is pasted below. No longer offered for new workflows.
      </Hint>
      <TextInput
