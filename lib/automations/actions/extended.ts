...
const applyDiscount = stub(
  'apply_discount',
  z.object({
    target: z.enum(['invoice']).default('invoice'),
    targetId: z.string().uuid().optional(),
    amount: z.number().optional(),
    percent: z.number().min(0).max(100).optional(),
    reason: z.string().optional(),
  }).passthrough(),
  { category: 'payments', label: 'Apply discount', description: 'Discount an invoice', icon: 'BadgePercent' },
)
...
const addLineItem = stub(
  ...
  { category: 'payments', label: 'Add line item', description: 'Add an item to an invoice (upsell)', icon: 'ListPlus' },
)
