/**
 * `proposal_due` time-based emitter.
 *
 * Fires when a proposal with status `'sent'` has an `expires_at` that
 * matches the lead-time configured on an active `proposal_due`
 * automation. e.g. an automation with `config.days = 3` fires on
 * `expires_at - 3 days`; `days = 0` fires on the expiry date itself.
 *
 * # Emit semantics
 *
 *   - One event per (proposal, days-lead-time, calendar day). Two
 ...
 *     configured lead-time" — without that, all `proposal_due`
 ...
 *     today is skipped. Across ticks this means each proposal gets at
 ...
 *
 * @module lib/automations/time-emitters/proposal-due
 */
...
/**
 * Render the `expires_at` value (a Postgres `date`) for a proposal
...
/**
 * Collect every (user, days) pair across active `proposal_due`
...
async function collectActiveLeadTimes(
  ...
  const { data, error } = await supabase
    ...
    .eq('status', 'active')
    .eq('trigger_type', 'proposal_due')

  if (error) throw new Error(`load proposal_due automations: ${error.message}`)
 ...
 * (proposal, days_until_due) bucket? Prevents re-emit across ticks
...
/**
 * Find proposals for `userId` that should fire `proposal_due` today for
 * the given `days` lead-time. Only `sent` proposals are candidates —
...
/**
 * Emit a single `proposal_due` event for a proposal at a given lead-time.
...
async function emit(
  ...
  if (error) throw new Error(`emit proposal_due: ${error.message}`)
}
 ...
 */
export const proposalDueEmitter: TimeEmitter = {
  type: 'proposal_due',
  async run(supabase) {
    ...
    for (const [userId, leadTimes] of grouped) {
      for (const days of leadTimes) {
        ...
        for (const proposal of candidates) {
          if (await alreadyEmittedToday(supabase, proposal.id, days, dayStart)) {
            continue
          }
          await emit(supabase, proposal, days)
          emitted += 1
        }
      }
    }
