    ...
    <div className="space-y-4">
      {!vendors || vendors.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-body text-text-muted mb-3">No contacts assigned yet.</p>
          <ContactPopover
            excludeIds={[]}
            onAdd={(id) => addVendor.mutate(id)}
          >
            <button className="text-body text-gray-700 border border-border rounded-control px-3 py-1.5 hover:bg-gray-50 transition cursor-pointer">
              + Add Contact
            </button>
          </ContactPopover>
        </div>
        ...
        <>
          ...
          <ContactPopover
            ...
            onAdd={(id) => addVendor.mutate(id)}
          >
            <button className="w-full text-body text-gray-700 border border-border rounded-control px-3 py-1.5 hover:bg-gray-50 transition cursor-pointer">
              + Add Contact
            </button>
          </ContactPopover>
        </>
